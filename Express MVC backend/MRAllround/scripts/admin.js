...
function renderUsersTable(users) {
  ...
  users.forEach(user => {
    ...
    entry.innerHTML = `
      ...
      <div><strong class="onscreenText adminhtmlPassword:"></strong><p class="onscreenText${user.id}" style="display: inline; color: transparent">${user.password}</p></div>
...
async function saveUserEdition(userId, userPassword) {
  const onscreenUserId = "onscreenText" + userId
  ...
  if (res.ok) {
    ...
    document.getElementsByClassName(onscreenUserId)[2].innerText = oldUser.email;
...
for (let i = 0; i < menuList.length; i++) {
  menuList[i].addEventListener('click', () => {
    document.getElementById('contentPageDropdown-toggle').innerHTML = menuList[i].innerHTML
...
function renderCardsTable(cards) {
  ...
  cards.forEach(card => {
    ...
    entry.innerHTML = `
      ...
      <div><strong class = "onscreenText adminhtmlCardPublic:"></strong><p class = "onscreenText${card.id}" style="display: inline;" onclick="this.focus()" >${card.public}</p></div>
      <div><strong class = "onscreenText adminhtmlCardNumber:"></strong><p class = "onscreenText${card.id}" style="display: inline;" onclick="this.focus()" >${card.number}</p></div>
...
function cancelCardEdition(cardId, titleDEU, titleENG, titleMKD, descriptionDEU, descriptionENG, descriptionMKD, public, number, picture) {
  ...
  elements[6].innerText = public;
  elements[7].innerText = number;
  elements[8].innerText = picture;
...
async function saveCardEdition(cardId) {
  ...
  const updatedCard = {
    ...
    public: elements[6].innerText,
    number: elements[7].innerText,
    picture: elements[8].innerText,
    id: cardId,
  };
  ...
  try {
    ...
    if (oldCard) {
      ...
      elements[6].innerText = oldCard.number;
      elements[7].innerText = oldCard.public;
